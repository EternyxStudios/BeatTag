...
async function renderPublicProfile(userId, fallbackName = 'BeatTag User') {
  ...
  try {
    ...
    screenEl.innerHTML = `
      <section class="panel public-profile-head premium-profile-head">
...
const SHOP = [
  ...
  { id:'boost_24h', icon:'↟', name:'Challenge Boost', desc:'Boost one of your challenges for 24 hours.', cost:120, duration:'Consumable', category:'Boosts', consumable:true, featured:true, rarity:'Power-up' },
  { id:'streak_shield', icon:'◈', name:'Streak Shield', desc:'A collectible power-up reserved for streak protection.', cost:160, duration:'Consumable', category:'Boosts', consumable:true, rarity:'Power-up' }
];
...
async function claimDailyReward(){
  ...
  try{
    ...
    renderShop();
  }catch(err){ console.error('Daily reward error:',err); toast(err?.message||'Could not claim the daily reward. Run the BeatTag Shop database upgrade first.'); if(button) button.disabled=false; }
}
...
async function buyItem(id){
  ...
  }catch(err){ console.error('Shop purchase error:',err); toast(String(err?.message||'Purchase failed.')); if(button) button.disabled=false; }
}
...
function renderShopItem(container,item,compact=false){
  ...
  const action=item.consumable
    ? (owned?`<button class="secondary" onclick="${item.id==='boost_24h'?'openBoostPicker()':`toast('Stored in your inventory.')`}">${item.id==='boost_24h'?`Use Boost (${quantity})`:`Owned (${quantity})`}</button>`:`<button class="primary" data-buy-item="${item.id}" onclick="buyItem('${item.id}')">🪙 ${item.cost}</button>`)
  ...
  container.insertAdjacentHTML('beforeend',`<article class="shop-item ${compact?'featured-item':''}"><div class="shop-item-top"><div class="shop-icon">${item.icon}</div><span class="shop-rarity">${escapeHTML(item.rarity||'')}</span></div><h3>${escapeHTML(item.name)}</h3><p>${escapeHTML(item.desc)}</p><div class="shop-meta"><span>${escapeHTML(item.category)}</span><span>${escapeHTML(item.duration)}</span></div>${action}</article>`);
}
...
function renderShop(){
  ...
  screenEl.innerHTML=`
    ...
    <section class="daily-reward-card"><div class="daily-reward-icon">✦</div><div><strong>Daily Reward</strong><p>Claim your daily BeatCoins and keep your challenge momentum going.</p></div><button id="dailyRewardBtn" class="primary" ${dailyClaimed ? 'disabled' : ''} onclick="claimDailyReward()">${dailyClaimed ? '✓ Claimed Today' : 'Claim +30'}</button></section>
    <div class="shop-tabs" role="tablist"><button class="active" onclick="filterShopCategory('All',this)">All</button><button onclick="filterShopCategory('Frames',this)">Frames</button><button onclick="filterShopCategory('Badges',this)">Badges</button><button onclick="filterShopCategory('Themes',this)">Themes</button><button onclick="filterShopCategory('Boosts',this)">Boosts</button></div>
    <div class="section-title"><h2>Featured</h2><span class="muted">Curated for BeatTag</span></div><div class="shop-grid" id="featuredShop"></div>
    ...
    <div class="section-title"><h2>Store</h2><span class="muted">${SHOP.length} items</span></div><div class="shop-grid" id="shop"></div>
  ...
  SHOP.filter(item=>item.featured).forEach(item=>renderShopItem($('#featuredShop'),item,true));
  SHOP.forEach(item=>renderShopItem($('#shop'),item,false));
  renderInventoryItems();
}
function filterShopCategory(category,button){
  document.querySelectorAll('.shop-tabs button').forEach(btn=>btn.classList.remove('active')); button?.classList.add('active');
  const container=$('#shop'); if(!container) return; container.innerHTML=''; SHOP.filter(item=>category==='All'||item.category===category).forEach(item=>renderShopItem(container,item,false));
}
function renderInventoryItems(){
  ...
  container.innerHTML=ownedItems.map(item=>`<div class="inventory-chip"><span>${item.icon}</span><div><strong>${escapeHTML(item.name)}</strong><small>${item.consumable?`${inventoryQuantity(item.id)} available`:'Unlocked'}</small></div></div>`).join('');
}
