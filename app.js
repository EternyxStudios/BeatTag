...
function creatorLeaderboard(limit = 20) {
  ...
  state.challenges.forEach(c => {
    ...
    if (c.creatorName) entry.name = c.creatorName;
    if (c.creatorHandle) entry.handle = c.creatorHandle;
    if (c.creatorAvatarUrl) entry.avatarUrl = c.creatorAvatarUrl;
...
function renderLeaderboard() {
  ...
  leaders.forEach((creator, index) => {
    ...
    row.innerHTML = `
      ...
      <span class="leaderboard-avatar">${creator.avatarUrl ? `<img src="${escapeAttr(creator.avatarUrl)}" alt="${escapeAttr(creator.name || 'BeatTag creator')} profile photo">` : (escapeHTML(creator.name)[0] || 'B')}</span>
...
function renderExplore() {
  ...
  if (me && currentUserId) {
    state.challenges.forEach(c => {
      if (c.creator === currentUserId) {
        c.creatorName = me.name || c.creatorName;
        c.creatorHandle = me.handle || c.creatorHandle || '';
        c.creatorAvatarUrl = me.avatarUrl || c.creatorAvatarUrl || '';
      }
    });
  }
