...
async function sendPasswordReset() {
  ...
  try {
    ...
    const { error } = await supabaseClient.auth.resetPasswordForEmail(email, {
      redirectTo: 'https://eternyxstudios.github.io/BeatTag/'
    });
    if (error) throw error;
...
n.querySelector(
  ...
  `🔗 ${c.attempts || 0} ${(c.attempts || 0) === 1 ? 'attempt' : 'attempts'} • Generation ${c.generation}${parentText}${taggedNames}`;
  return n;
}
...
function renderProfile() {
  ...
  screenEl.innerHTML = `
    ...
    <section class="panel profile-head ${p.equippedTheme ? `profile-theme-${escapeAttr(p.equippedTheme)}` : ''}">
      ...
      <button
        class="secondary settings-profile-btn"
...
async function requestAccountDeletion() {
  const first = confirm('Delete your BeatTag account? Your profile will be deactivated and your active challenges will be removed.');
  if (!first) return;
        ...
        state.notifications.length
          ? state.notifications.map(n => `
              <div class="notification-item ${n.read ? '' : 'unread'}">
                <button class="notification-main" onclick="markNotificationRead('${escapeAttr(n._id)}')">
