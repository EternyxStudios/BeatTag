...
async function renderPublicProfile(userId, fallbackName = 'BeatTag User') {
  ...
  try {
    const { data: publicProfile, error } = await supabaseClient
      .from('profiles')
      .select('*')
      .eq('id', userId)
      .maybeSingle();
    ...
    const username = publicProfile?.username
      ? '@' + String(publicProfile.username).replace(/^@/, '')
      : '';

    const likes = posts.reduce(
      (sum, c) => sum + Object.keys(c.likes || {}).length,
      0
    );

    const attempts = posts.reduce(
    ...
    screenEl.innerHTML = `
      <section class="panel public-profile-head">
        ...
        ${publicProfile?.bio ? `
          <p class="profile-bio">${escapeHTML(publicProfile.bio)}</p>
        ` : ''}

        ${profileLinksHTML(
          publicProfile?.website_url_1 || '',
          publicProfile?.website_url_2 || ''
        )}
      ...
      <div class="section-title">
        <h2>${escapeHTML(displayName)}'s Challenges</h2>
      </div>
...
`;
  ...
  try {
    ...
    box.innerHTML = `<div class="empty">Reports load nahi hue.</div>`;
  }
}
...
async function adminDismissReport(reportId) {
  ...
  try {
    ...
    toast(err.message || 'Report dismiss nahi hua.');
  }
}
...
async function adminRemoveChallenge(reportId, challengeId) {
  ...
  if (!confirm('Is challenge ko BeatTag se remove karna hai?')) return;
  ...
  try {
    ...
    toast(err.message || 'Challenge remove nahi hua.');
  }
}
...
async function adminModerateUser(userId, action) {
  ...
  const reason = prompt(
    'Reason likho (adult content, graphic content, harassment, etc.):',
...
async function saveProfile() {
  ...
  if (username.length < 3) {
    toast('Handle minimum 3 characters ka rakho.');
    return;
  }

  if (countWords(bio) > 200) {
    toast('Bio maximum 200 words ka ho sakta hai.');
    return;
  }
  ...
  if (rawLink1 && !link1) {
    toast('Link 1 valid website URL nahi hai.');
    return;
  }

  if (rawLink2 && !link2) {
    toast('Link 2 valid website URL nahi hai.');
    return;
  }
  ...
  try {
    ...
    toast(err.message || 'Profile save nahi hua. Run supabase_upgrade.sql first.');
  }
}

async function requestBeatTagNotifications() {
  if (!('Notification' in window)) {
    toast('Is browser me notifications supported nahi hain.');
    return;
  }
  ...
  try {
    ...
    if (permission === 'granted') {
      ...
      await showBeatTagNotification(
        ...
        'Naye challenges ki live notification milegi jab BeatTag browser/PWA active ho.'
      );
      toast('Notifications enabled ✅');
    } else {
      toast('Notification permission allow nahi hui.');
    }
  } catch (err) {
    console.error('Notification permission error:', err);
    toast('Notifications enable nahi hui.');
  }
}
